...
class FeatureFusionNetwork(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_featurefusion_layers=2,
        ...
        decoderCFA_layer = DecoderCFALayer(d_model, nhead,  dropout)
...
class FeatureFusionLayer(nn.Module):
    ...
    def forward_post(self, src1, src2,
        ...
        src12 ,wegiht1= self.multihead_attn1(query=self.with_pos_embed(src1, pos_src1),
                                   key=self.with_pos_embed(src2, pos_src2),
                                   value=src2, attn_mask=src2_mask,
                                   key_padding_mask=src2_key_padding_mask,need_weights=True)
        src22,weight2  = self.multihead_attn2(query=self.with_pos_embed(src2, pos_src2),
                                   key=self.with_pos_embed(src1, pos_src1),
                                   value=src1, attn_mask=src1_mask,
                                   key_padding_mask=src1_key_padding_mask,need_weights=True)
