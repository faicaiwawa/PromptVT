    ...
    for key, data in output.items():
        ...
        if key == 'all_scores':
            ...
            if isinstance(data[0], dict):
                ...
                for obj_id, d in data_dict.items():
                    timings_file = '{}_{}_model_time.txt'.format(base_results_path, obj_id)
                    save_model_time(timings_file, d)
            else:
                timings_file = '{}_model_time.txt'.format(base_results_path)
                save_model_time(timings_file, data)
