...
'''efttrack_vot20 class'''


class efttrack_vot20(object):
    def __init__(self, tracker_name='PromptVT', para_name='baseline'):
        # create tracker
...
def run_vot_exp(tracker_name, para_name, vis=False):
    ...
    save_root = os.path.join('/qiuyang/efttrack_s1/external/vot20/efttrack/logs', para_name)
    if vis and (not os.path.exists(save_root)):
        os.mkdir(save_root)
    tracker = efttrack_vot20(tracker_name=tracker_name, para_name=para_name)
    handle = vot.VOT("mask")
